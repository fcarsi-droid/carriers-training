...
export async function GET(req) {
  ...
  if (!isAdminOrMaster(user)) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
...
export async function POST(req) {
  ...
  try {
    ...
    // Only master can create admins or other masters
    const assignedRole = (user.role === 'master') ? (role || 'user') : 'user'
    ...
    const res = await query(
      ...
      [name, first_name.trim(), last_name.trim(), username, carrier, hash, assignedRole]
    )
